...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    hWnd = CreateWindowExW(WS_EX_NOREDIRECTIONBITMAP, // NULL, // WS_EX_LAYERED,
                           className, titleName,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_NCACTIVATE:  // Do not draw the nonclient area.
        break;
    case WM_NCPAINT:  // Do not draw the nonclient area.
        break;
...
IDWriteTextLayout2* pDWriteTextLayout;
...
void CreateDeviceIndependentResources() {
    ...
    static std::wstring text = L"Hello 你好world!";
    ...
    if (FAILED(hr)) { throw; }
    text_layout->QueryInterface(&pDWriteTextLayout);

    IDWriteFontFallbackBuilder* font_fallback_builder;
    pDWriteFactory->CreateFontFallbackBuilder(&font_fallback_builder);

    DWRITE_UNICODE_RANGE range = { 0, -1 };
    const wchar_t* family_names[2] = { L"DengXian", L"Consolas" };
    font_fallback_builder->AddMapping(&range, 1, family_names, 2);

    IDWriteFontFallback* font_fallback;
    font_fallback_builder->CreateFontFallback(&font_fallback);
    pDWriteTextLayout->SetFontFallback(font_fallback);
}
...
void OnPaint() {
    ...
    pD2DDeviceContext->Clear(D2D1::ColorF(D2D1::ColorF::White, 0.5));
