...
void OnPaint();
void OnSize(int, int);
void OnDpiChange(unsigned dpi);
void OnScale(float scale, int x, int y);
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    DPI_AWARENESS_CONTEXT old_dpi = SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DPICHANGED:
        {
            //unsigned y_dpi = HIWORD(wParam);
            //unsigned x_dpi = LOWORD(wParam);
            static unsigned dpi_old = 96;
            unsigned dpi_new = LOWORD(wParam);
            OnScale(dpi_new / (float)dpi_old, 0, 0);
            //OnDpiChange(dpi_new);  // Both are ok.
            dpi_old = dpi_new;
...
void OnPaint() {
    ...
    pD2DDeviceContext->Clear(D2D1::ColorF(D2D1::ColorF::White, 1.0));
...
void OnDpiChange(unsigned dpi) {
    pD2DDeviceContext->SetTarget(pD2DTargetBitmap);
    pD2DDeviceContext->SetDpi(dpi, dpi);
    pD2DDeviceContext->SetTarget(nullptr);
}
